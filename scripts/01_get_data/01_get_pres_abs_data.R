# get gridded presence absence data from polygon range maps
# Lydia Soifer
...
# mammal data
data = vect("./../data/ranges/MAMMALS_TERRESTRIAL_ONLY/MAMMALS_TERRESTRIAL_ONLY.shp")
...
data = vect("./../data/ranges/AMPHIBIANS/AMPHIBIANS.shp")
gc()
...
data = vect("./../data/ranges/reptiles/Gard_1_7_ranges.shp")
...
# seasonal: all resident, breeding, non-breeding
# keep cells in matrix where richness = 0
presab_rept = lets.presab(data_reproj_valid, resol = 111000, 
                          crs = "+proj=cea +datum=WGS84", crs.grid = "+proj=cea +datum=WGS84",
                          count = T, cover = 0,
                          xmn = -20592508, xmx = 20588492, ymn = -5743602, ymx = 6573398,
                          remove.cells = FALSE)
...
# write occr
write.csv(presab_rept$Presence_and_Absence_Matrix, "data/pres_abs_cover0/pres_abs_rept.csv",row.names = F)
# write species list
write.csv(data.frame(species=presab_rept$Species_name), "data/pres_abs_cover0/sp_list_rept.csv",row.names = F)
...
data_layers = st_layers("./../data/ranges/BOTW_2022.2/BOTW.gdb")

data_proxy = vect("./../data/ranges/BOTW_2022.2/BOTW.gdb", layer = "All_Species", proxy = T)
