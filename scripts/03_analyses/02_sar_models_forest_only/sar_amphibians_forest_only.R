...
# subset data to forested areas

# option 1: subset by biome
unique(amph$biome)
amph.forests = amph %>% 
  filter(grepl("Forests", biome))

ggplot(amph.forests) +
  geom_boxplot(aes(x = biome, y = veg_den))
...
ggplot(amph) +
  geom_boxplot(aes(x = biome, y = veg_complexity)) +
  theme(axis.text = element_text(angle = 270))

# urbanization might be impacting veg density
veg = amph %>% 
  dplyr::select(x,y,veg_den, veg_complexity, canopy_height, biome) %>% 
...
# ggplot(biorealm) +
#   geom_spatvector(aes(fill = BIOME_NAME)) +
#   coord_sf(crs=4326)
