...
f = formula(vert.mean.ses ~ log_elev + log_clim_velocity + precip_dry + log_precip_sea + precip_wet + tmax_warm + tmin_cold + veg_complexity)
...
save(sesvert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, sesvert.dredge, sesvert.avg,
     file = "results/sar_mods_forest_only/mammals/mammals_sar_sesvert2.RData")
...
sesvert.scale.sub$pred8 = as.vector(best_mods_pred[[8]][["pred"]])
sesvert.scale.sub$pred9 = as.vector(best_mods_pred[[9]][["pred"]])
sesvert.scale.sub$pred10 = as.vector(best_mods_pred[[10]][["pred"]])


sesvert.scale.sub = sesvert.scale.sub %>% 
  mutate(vert.mean.ses.future = rowMeans(select(sesvert.scale.sub, pred1:pred10)))
...
save(sesvert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, sesvert.dredge, sesvert.avg, 
     best_mods_pred, sesvert.scale.sub, file = "results/sar_mods_forest_only/mammals/mammals_sar_sesvert2.RData")
...
save(vert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2,
     vert.dredge, vert.avg, best_mods_pred, file = "results/sar_mods_forest_only/mammals/mammals_sar_vertmean2.RData")
...
save(vert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, vert.dredge,
     vert.avg, best_mods_pred, vert.scale.sub, file = "results/sar_mods_forest_only/mammals/mammals_sar_vertmean2.RData")
