...
# This file must be run on hipergator
...
pts = data.frame(lon = c(-89.54, -91.08,-75.57, 
                         137.77, 10.50, 117.69,112.5),
                 lat = c(50.75,37.69,-5.24, 
                         50.8, 48.33, 4.95,-1.5),
                 region = c("canada_boreal", "mark_twain_nf", "pacaya_samiria",
                            "russia_boreal", "europe_temperate", "danum_valley", "borneo2"))
...
gedi = list()
for(p in 1:nrow(pts)) {
  g = get_gedi_tiles(e = pts[p], fpath = "/orange/scheffers/lydia.soifer/big_data/vegetation/gedi_L2b/tiles/", unzip = T)
  gedi[[p]] = g
}
...
test = fread("data/derivative_data/gedi_filtered/gedi_europe_temperate.csv")
test[,month := month(as.Date(date))]
test.jan = test[month == 1,]
test.jan = test[, year := year(as.Date(date))]
test.jan.sub = test.jan[year < 2023,]
