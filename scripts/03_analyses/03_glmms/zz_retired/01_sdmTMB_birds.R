...
# scale the data
dat = dat.t %>%
  mutate_at(.vars = vars(canopy_height:clim_velocity, elev, veg_den, veg_complexity, log_precip_dry, log_clim_velocity, precip_warm), .funs = scale)
...
saveRDS(list(mesh = mesh), "results/sdmTMB_models2/birds_sesvert.rds")
#out = readRDS("results/sdmTMB_models2/birds_sesvert.rds")
#mesh = out$mesh
...
saveRDS(list(mesh = mesh, compMods_aic = compMods_aic),  "results/sdmTMB_models2/birds_sesvert.rds")

compMods_aic = out$compMods_aic
compMods_aic$modsel
sanity(compMods_aic$modlist[[3]])
summary(compMods_aic$modlist[[3]])
tidy(compMods_aic$modlist[[3]])
...
saveRDS(list(mesh = mesh, compMods_aic = compMods_aic, bestmod = bestmod), "results/sdmTMB_models2/birds_sesvert.rds")
...
plot_resids(mod = bestmod, response_var = "vert.mean.ses", 
            fpath = paste0("figures/residual_checks/sdmTMB2/birds/",fname_end), integer_response = F)
...
# # 
# # save(m.final, pred, pred.f, file = "results/sdmTMB_models/birds_sesvert.RData")
# # 
# # 
# # # MEAN VERTICALITY --------------------------------------------------------
# # 
...
# #   coord_sf(crs = "+proj=cea +datum=WGS84")
# # 
# # save(m.final, pred, pred.f, file = "results/sdmTMB_models/birds_meanvert.RData")
