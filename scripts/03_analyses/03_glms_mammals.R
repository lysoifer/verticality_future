...
mammals = read.csv("data/derivative_data/gridcell_data/mammals_comdat/mammals_comdat_parallel.csv")
...
# sesvert.t = sesvert.t %>% 
#   dplyr::select(!c(clim_velocity, elev, temp_sea))
...
# SES model
# f = formula(vert.mean.ses ~ log_elev + log_clim_velocity + precip_dry + precip_sea + precip_wet + temp_diu + log_temp_sea + veg_complexity)
# f.realm = formula(vert.mean.ses ~ 0 + realm/(log_elev + log_clim_velocity + precip_dry + precip_sea + precip_wet + temp_diu + log_temp_sea + veg_complexity))

f = formula(vert.mean.ses ~ log_elev + log_clim_velocity + precip_dry + log_precip_sea + precip_wet + tmax_warm + I(tmax_warm^2) + tmin_cold + veg_complexity)
...
#save(mod.rs, file = "results/mammals_sar_mod_rs.RData")
...
#save(mod.idist, file = "results/mammals_sar_mod_idist.RData")
...
#save(mod.idist2, file = "results/mammals_sar_mod_idist2.RData")
...
# save(sesvert.full, mod.rs, mod.idist, mod.idist2, sesvert.dredge, sesvert.avg, file = "results/mammals_sar_sesvert.RData")

save(sesvert.full, wts.rs, wts.idist, wits.idist2, mod.rs, mod.idist, mod.idist2, sesvert.dredge, sesvert.avg,
     file = "results/mammals_sar_sesvert2.RData")
...
env.future.df = sesvert.scale %>% 
  dplyr::select(x, y, log_elev, veg_complexity, log_clim_velocity) %>% 
  right_join(env.future.df, by = c("x", "y")) %>% 
  dplyr::select(x,y,tmax_warm, tmin_cold, precip_wet, precip_dry, log_precip_sea, log_elev, 
                veg_complexity, log_clim_velocity)
...
# save(sesvert.full, mod.rs, mod.idist, mod.idist2, sesvert.dredge, sesvert.avg, best_mods_pred, file = "results/mammals_sar_sesvert.RData")
# load("results/mammals_sar_sesvert.RData")
...
# join results to env data and current sesvert
sesvert.scale.sub %>% 
  dplyr::select(x,y,vert.mean.ses.future) %>% 
  right_join(sesvert.scale, by = c("x", "y"))

save(sesvert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, sesvert.dredge, sesvert.avg, 
     best_mods_pred, sesvert.scale.sub, file = "results/mammals_sar_sesvert2.RData")
...
# vert.t = vert.t %>% 
#   dplyr::select(!c(clim_velocity, elev, temp_sea, precip_sea, log_precip_dry))

vert.t = vert.t %>% 
  dplyr::select(!c(clim_velocity, elev, precip_sea, log_precip_dry, log_precip_wet))
...
f = formula(vert.mean ~ log_elev + log_clim_velocity + tmin_cold + tmax_warm + precip_dry + log_precip_sea + precip_wet + veg_complexity)
...
#save(mod.rs, file = "results/mammals_vertmean_sar_mod_rs.RData")
...
#save(mod.idist, file = "results/mammals_vertmean_sar_mod_idist.RData")
...
#save(mod.idist2, file = "results/mammals_vertmean_sar_mod_idist2.RData")
...
# save(vert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, vert.dredge, vert.avg, file = "results/mammals_sar_vertmean.RData")
...
#          precip_sea = (log_precip_sea - mean(vert.t$log_precip_sea)) / sd(vert.t$log_precip_sea),
...
#   dplyr::select(x, y, log_elev, veg_complexity, log_clim_velocity) %>% 
#   right_join(env.future.df, by = c("x", "y")) %>% 
#   dplyr::select(x,y,temp_diu, precip_wet, precip_dry, log_precip_sea, log_elev, veg_complexity, log_clim_velocity, log_temp_sea)
...
best_mods_pred <- foreach(r=1:nrow(vert.dredge.sel), .packages = c("spatialreg", "spdep", "broom")) %dopar% {
  ...
  list(row = r, mod = msar.err, moran = moran_test, pred = pred, r2 = r2)
}

stopCluster(cl)

# save(vert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, vert.dredge, vert.avg, best_mods_pred, file = "results/mammals_sar_vertmean.RData")

save(vert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, vert.dredge, vert.avg, best_mods_pred, file = "results/mammals_sar_vertmean2.RData")
...
# check spatial autocorrelation
lapply(best_mods_pred, "[[", 3) # all okay
r2 = mean(sapply(best_mods_pred, "[[", 5)) # all okay
...
vert.avg.full %>% 
  ...
  geom_pointrange() + 
  geom_vline(xintercept = 0, linetype = "dashed") +
  annotate(geom = "text", x = 0.01, y = 8, label = paste0("R\u00b2 = ", r2)) +
  scale_color_manual(values = c("black", "red3")) +
  scale_y_discrete("") +
  ggtitle("Mean Verticality: Mammals (moura)") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5))
...
mp =  vert.scale.sub %>% 
  ...
  #ggtitle("Mammals mean verticality (moura)") +
  facet_wrap(facets = ~Time, nrow = 2) +
  theme_classic() +
  theme(axis.text = element_blank(),
        axis.title = element_blank(),
...
hist + mp + plot_layout(design = "12
                                  12", widths = c(5,20), heights = c(0.5, 1)) +
  plot_annotation(title = "Mammals: mean verticality (moura)")

save(vert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, vert.dredge,
     vert.avg, best_mods_pred, vert.scale.sub, file = "results/mammals_sar_vertmean2.RData")
