...
f = formula(vert.mean.ses ~ log_elev + log_clim_velocity + tmax_warm + I(tmax_warm^2) + tmin_cold + precip_dry + log_precip_sea + precip_wet + veg_complexity)
...
save(sesvert.full, mod.rs, mod.idist, mod.idist2, sesvert.dredge, sesvert.avg, i, 
     file = "results/sar_mods_forestOnly_forestSES/mammals/mammals_sar_sesvert.RData")
...
save(sesvert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, sesvert.dredge, sesvert.avg, best_mods_pred, 
     sesvert.scale.sub, i, file = "results/sar_mods_forestOnly_forestSES/mammals/mammals_sar_sesvert.RData")
...
vert.t = vert.t %>% 
  dplyr::select(!c(clim_velocity, elev, precip_sea, log_precip_wet, log_precip_dry, log_veg_complexity))
...
# vert mean model
f = formula(vert.mean ~ log_elev + log_clim_velocity + tmax_warm + I(tmax_warm^2) + tmin_cold + precip_dry + log_precip_sea + precip_wet + veg_complexity)
...
## run dredge to iterate through all possible variable combos
## tests all combinations of predictor variables (511 models)
...
save(vert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, vert.dredge, vert.avg, vert.weights,
     file = "results/sar_mods_forestOnly_forestSES/mammals/mammals_sar_meanvert.RData")

# predict model
pred = predict(vert.avg, newdata = vert.scale, type = "response")

# pseudo-r2 (see https://onlinelibrary.wiley.com/doi/full/10.1111/j.1466-8238.2007.00334.x)
r2 = cor(pred, vert.scale$vert.mean)
...
env.future.df = vert.scale %>% 
  ...
  dplyr::select(rownames, x,y, tmin_cold, tmax_warm, precip_wet, precip_dry, log_precip_sea,
...
pred.future = MuMIn:::predict.averaging(vert.avg, newdata = env.future.df, type = "response", pred.type = "TS")
...
save(vert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, vert.dredge, vert.avg, best_mods_pred,
     file = "results/sar_mods_forestOnly_forestSES/reptiles/reptiles_sar_meanvert.RData")
...
vert.scale.sub = vert.scale.sub %>% 
  mutate(vert.mean.future = rowMeans(select(vert.scale.sub, pred1:pred5)))
