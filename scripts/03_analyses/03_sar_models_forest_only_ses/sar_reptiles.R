...
repts = read.csv("data/derivative_data/gridcell_data/reptiles_comdat/rept_comdat_parallel_forestsOnly.csv")
...
save(sesvert.full, mod.rs, mod.idist, mod.idist2, sesvert.dredge, sesvert.avg, 
...
save(sesvert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, sesvert.dredge, sesvert.avg, best_mods_pred, 
     sesvert.scale.sub, file = "results/sar_mods_forestOnly_forestSES/reptiles/reptiles_sar_sesvert.RData")
...
env.future.df = vert.scale %>% 
  dplyr::select(x, y, log_elev, veg_complexity, log_clim_velocity) %>% 
  right_join(env.future.df, by = c("x", "y")) %>% 
  dplyr::select(x,y, tmin_cold, tmax_warm, log_precip_wet, log_precip_dry, precip_sea, log_elev, veg_complexity, log_clim_velocity)
