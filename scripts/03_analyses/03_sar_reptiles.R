...
repts = read.csv("data/derivative_data/gridcell_data/reptiles_comdat/rept_comdat_parallel.csv")

#  * - data check --------------------------------------------------------------

## SES vert model
sesvert = repts[,c("x", "y", "vert.mean.ses", "biome", "realm", env_vars)] %>% 
  drop_na()
...
#save(mod.rs, file = "results/reptiles_sar_mod_rs.RData")
...
#save(mod.idist, file = "results/reptiles_sar_mod_idist.RData")
...
#save(mod.idist2, file = "results/reptiles_sar_mod_idist2.RData")
...
# save(sesvert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, sesvert.dredge, sesvert.avg, file = "results/reptiles_sar_sesvert.RData")

save(sesvert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, sesvert.dredge,
     sesvert.avg, file = "results/reptiles_sar_sesvert2.RData")
...
# save(sesvert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, sesvert.dredge, sesvert.avg, best_mods_pred, file = "results/reptiles_sar_sesvert.RData")
# load("results/reptiles_sar_sesvert.RData")
...
save(sesvert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, sesvert.dredge, 
     sesvert.avg, best_mods_pred, sesvert.scale.sub, file = "results/reptiles_sar_sesvert2.RData")
...
sesvert.avg.full %>% 
  ...
  scale_y_discrete("") +
  ggtitle("SES Mean Verticality: Reptiles (moura)") +
  theme_classic() +
...
## vert mean model
vert = repts[,c("x", "y", "vert.mean", "biome", "realm", env_vars)] %>% 
  drop_na()
...
vert.t = vert.t %>%
  dplyr::select(!c(clim_velocity, elev, log_precip_dry, log_precip_wet))
...
f = formula(vert.mean ~ log_elev + log_clim_velocity + tmin_cold + tmax_warm + precip_dry + precip_sea + precip_wet + veg_complexity)
...
#save(mod.rs, file = "results/reptiles_vertmean_sar_mod_rs.RData")
...
#save(mod.idist, file = "results/reptiles_vertmean_sar_mod_idist.RData")
...
#save(mod.idist2, file = "results/reptiles_vertmean_sar_mod_idist2.RData")
...
#save(vert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, vert.dredge, vert.avg, file = "results/reptiles_sar_vertmean.RData")
...
# save(vert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, vert.dredge, vert.avg, best_mods_pred, file = "results/reptiles_sar_vertmean.RData")
save(vert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, vert.dredge, vert.avg, best_mods_pred, file = "results/reptiles_sar_vertmean2.RData")
...
vert.avg.full %>% 
  ...
  scale_color_manual(values = c("black", "red3")) +
  scale_y_discrete("") +
  ggtitle("Mean Verticality: Reptiles (moura)") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5))


vert.scale.sub = vert.scale.sub %>% 
  mutate(vert.mean.future = rowMeans(select(vert.scale.sub, pred1:pred2)))
...
mp =  vert.scale.sub %>% 
  ...
  #ggtitle("Reptiles mean verticality (moura)") +
  facet_wrap(facets = ~Time, nrow = 2) +
  theme_classic() +
  theme(axis.text = element_blank(),
        axis.title = element_blank(),
...
hist + mp + plot_layout(design = "12
                                  12", widths = c(5,20), heights = c(0.5, 1)) +
  plot_annotation(title = "Reptiles: mean verticality (moura)")

save(vert.full, wts.rs, wts.idist, wts.idist2, mod.rs, mod.idist, mod.idist2, vert.dredge, 
     vert.avg, best_mods_pred, vert.scale.sub, file = "results/reptiles_sar_vertmean2.RData")
