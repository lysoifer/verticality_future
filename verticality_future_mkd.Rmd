...
```{r fig.id="mod-coefs", fig.width=160, fig.height=87.3, fig.cap.pre="Fig. ", fig.cap=" Coefficient estimates for models of mean and SES verticality with 95% confidence intervals. Red points indicate coefficients with 95% CIs that do not overlap zero."}
...
```{r fig.id="svc", fig.width=160, fig.height=143.95, fig.cap.pre="Fig. ", fig.cap="Spatially varying effect of canopy height on the mean and standard effect size (SES) of verticality for birds, mammals, reptiles, and amphibians. The spatially varying effect is the sum of the main effect of canopy and height and the spatially varying random effect at any given point. Larger positive numbers indicate a stronger positive effect of canopy height on verticality and larger negative numbers indicate a stronger negative effect of canopy height on verticality. Dark gray areas indicate locations where the effect of canopy height was not significant. Light gray areas indicate areas that are not wooded or that have fewer than five species."}
...
```{r fig.id="richness-mods", fig.width = 160, fig.height=115.6, fig.cap.pre="Fig. ", fig.cap="Slopes for linear models of richness as a function of SES or mean verticality across all biomes and for individual biomes. Hollow circles indicate non-significant relationships (p ≥ 0.05) and filled circles indicate significant relationships. R^2^ values are represented by the size of the circle. Purple = all biomes, gray = boreal biomes, orange = temperate biomes, green = tropical biomes. Flooded GS = Flooded grassland and savannas, Mediterranean FWS = Mediterranean forests, woodlands, and scrub, Montane GS = Montane grasslands and shrublands, Temperate BMF = Temperate broadleaf and mixed forests, Temperate CF = Temperate conifer forests, Temperate GSS = Temperate grasslands, savannas, and shrublands, Tropical CF = Tropical and subtropical conifer forests, Tropical DBF = Tropical and subtropical dry broadleaf forests, Tropical GSS = Tropical and subtropical grasslands, savannas, and shrublands, Tropical MBF = Tropical and subtropical moist broadleaf forests"}

include_graphics("figures/main_figs/vert_rich_lm_slopes.png", error = F)
```
...
include_graphics("figures/main_figs/fig5_vert_reldifs.png", error = F)
```
